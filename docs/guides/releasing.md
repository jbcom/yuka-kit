...
pull request is an encapsulation of already-merged, verified changes, so its
CI test job is intentionally skipped and trusted automation enables
merge-commit auto-merge. The release workflow then publishes only after a real
release tag exists and verifies the tagged package before npm publication with
provenance.
